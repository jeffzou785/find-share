...
**P2-0 统一 CLI**：`python3 -m src.pipeline.cli <subcommand>`，封装 13 个独立脚本。
13 个可独立运行的脚本，覆盖 bootstrap → 缓存预热 → 策略筛选 → 研报拉取 → RAG 检索 → 动态监控 全流程。

### 评分与监控层 `src/screening/`（P2 新增）
- **P2-1 评分层**（`scoring.py`）：线性模型 `growth*0.30 + valuation*0.20 + quality*0.20 + catalyst*0.15 + neglect*0.15 - risk_penalty`；权重写入 `screen_runs.config_json.score_weights`；只用于排序和 watch 分层，不覆盖硬风控
...
python3 -m src.pipeline.cli screen --period 2025Q1 --strategy overseas --enable-scoring          # 季报 + 评分
...
| 功能 | 完成内容 |
|------|---------|
| **P2-0 统一 CLI** ✅（2026-06-28）| `python3 -m src.pipeline.cli <subcommand>`，封装 13 个脚本（bootstrap/refresh/screen/strategy1/strategy3/reports/pdf/rag/baseline/monitor 等）|
| **P2-1 评分层**（`src/screening/scoring.py`）| 线性模型：`growth*0.30 + valuation*0.20 + quality*0.20 + catalyst*0.15 + neglect*0.15 - risk_penalty`；按策略差异化默认权重（consumer 不算 neglect，overseas 算）；子分缺失时权重重新归一化；`--enable-scoring` 写入 `metrics.score.final_score`；27 个测试覆盖 |
