...
### 策略层 `src/strategies/`
- `consumer_reversal.py`：策略一 + 反转判定 + P1.5-4 行业覆盖扩展（含化妆品/医美/纺服/服务消费/轻工）+ PE/PB 分位窗口参数化 + 经营质量过滤（现金流、扣非/归母净利背离、应收/存货增速背离、销售费用率抬升）
...
- P1：策略一消费池共 451 只，目前 31 只已补足历史 PE/PB，仍有 420 只待分批预热；后续建议按核心消费行业分批跑 `refresh --valuation-source akshare`。
...
python3 -m src.pipeline.cli refresh --valuation-source akshare --industries 食品饮料 家电 美容护理 --limit 50
python3 -m src.pipeline.cli screen --period 2026Q1 --strategy consumer --limit 50
