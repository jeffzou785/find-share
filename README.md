...
| 策略 | 逻辑 | 当前命中 |
|------|------|---------|
| **消费股反转** | 食品饮料/家电/美容护理/商贸零售/纺服/社会服务/轻工 + PE 分位<30%（**3y/5y/10y 可选**）+ 扣非同比>30% + **反转判定** + 经营质量过滤（现金流/扣非/应收-存货-费用率）| **4 只** |
...
- 估值/财务：`pe_pb_history` / `financials`（含应收账款、存货、销售费用）/ `financials_full`（新浪三表长格式）
...
### 策略层 `src/strategies/`
- `consumer_reversal.py`：策略一 + 反转判定 + 多字段消费行业准入（一级/二级行业 + `em2016` 日用化学品，覆盖美护/个护）+ PE/PB 分位窗口参数化 + 经营质量过滤（现金流、扣非/归母净利背离、应收/存货增速背离、销售费用率抬升）
...
完整的已完成/未完成交接清单见 [`PROGRESS.md`](PROGRESS.md)。

最近关键 run：

- P0 基线主 run：`20260704_112743_614f_2025A`，策略三 `hit=1`、`watch=6`，P0 审计已全 OK。
- 策略一历史估值小批次验证 run：`20260709_230515_2757_2025A`，31 只消费样本中 `rejected=30`、`data_missing=1`；说明已从 `pe_history_missing` 推进到真实估值/业绩/质量筛选原因。
- 策略一营运质量验证 run：`20260710_222101_f65a_2026Q1`，31 只消费样本 `rejected=30`、`data_missing=1`；主拒绝原因为 PE 分位过高 23 只、扣非增速不足 5 只。三项营运质量已全部写入 metrics，但因更早硬条件短路，尚无候选实际进入该分支。
- 策略一食品饮料预热 run：`20260711_111811_eadf_2026Q1`，首批 50 只均完成历史估值和财务补齐，筛选结果 `rejected=50`（PE 分位过高 30、扣非增速不足 15、非拐点/趋势 5），无数据缺失。
- 策略一美护/个护预热 run：`20260711_112734_afd4_2026Q1`，20 只均完成历史估值和财务补齐，筛选结果 `rejected=20`（PE 分位过高 12、扣非增速不足 4、非拐点/趋势 4），无数据缺失。
- 策略一纺织服装预热 run：`20260711_113042_b6e2_2026Q1`，50 只完成财务补齐、49 只历史估值达标；筛选结果 `rejected=46`、`data_missing=4`，主拒绝原因为 PE 分位过高 34 只。
- 策略二A ground truth 复盘 run：`20260709_224758_d4a5_2025A`，已生成 `data/exports/pharma_ground_truth_review.md` / `.csv`。
- 策略三 parser 质量池：`python3 -m src.pipeline.cli parser-review --year 2025` 已生成 `data/exports/overseas_parser_quality_review.md` / `.csv`。

已完成的数据补齐：

- `$a-stock-data` 补数入口已覆盖最近缺失池：32 只股票财务/估值补齐成功，默认补 2024/2025 年报与 2026Q1，不补 2023。
- 策略一历史估值入口已打通：`refresh --valuation-source akshare` 可用 AkShare/东财历史 PE/PB 补 3/5/10 年分位；首批 31 只消费样本已补到 700~2000+ 条 PE/PB 历史。
- 策略一消费池行业映射已校正：东财 `em2016` 的“日用化学品”纳入美护/个护入口，东财实际标签“纺织服装”“休闲、生活及专业服务”纳入纺服/服务消费入口。当前消费池为 684 只，199 只历史 PE/PB 样本达标，剩余 485 只待预热；食品饮料、家电、美护/个护、纺服各已完成一批。
- `refresh` 的历史样本状态改为按 DuckDB 持久化后的去重行数判定；`insufficient` 与策略的 `pe_history_missing` 口径一致，不再把接口重复日期造成的假充足样本报为成功。纺服批次中 `000955`、`001312`、`002875`、`300591` 已留在补数队列。
- 策略一财务窗口固定为最近 9 期（2024Q1 至 2026Q1），足够计算 TTM 同比，不再主动抓取 2023 年数据；运行时会按 `period` 截断财务序列，历史年报/季报回放不会误用未来财报。扣非缺失时可用归母/净利做代理 triage，但只能进入 watch，不能直接 hit。
- 策略一营运质量已接入新浪三表：应收账款、存货、销售费用持久化到 `financials`；按去年同报告期判断同比背离，避免把季节性当作风险。应收/存货增速分别高出收入 20/30pp 进入 watch，达到 50/80pp 拒绝；销售费用率上升 3pp/8pp 对应 watch/reject。字段尚未回填时保留原结果，并在 metrics 留下 `missing` 审计值。
- 首批 31 只消费样本已全部回填三项营运质量字段。按 2026Q1 metrics 预演：30 只可同比比较；应收异常 watch-or-reject 14 只（其中 7 只达到 reject）、存货 4/1 只、销售费用率 2/1 只。该分布只用于下一轮人工标注和阈值校准，暂不据此放宽或提高阈值。
...
- 2025 年 parser 质量池当前为：本地年报 PDF 85 份、已入库海外收入 71 只、待复核问题 33 条（P1=27，P2=6）。
- A/H 映射已有第一批 5 条：药明康德、康龙化成、泰格医药、君实生物、百济神州，可供 `$global-stock-data` 港股扩展池继续接入。

当前剩余主线：

- P1：策略一消费池共 684 只，目前 199 只已补足历史 PE/PB，仍有 485 只待分批预热；下一批为服务消费，之后继续轻工制造和商贸零售，并重试 `insufficient` 队列。
- P1：策略二A ground truth 复盘已具备工具，下一步继续补真实集采事件，优先处理 `positive_label_missed` 中的 VBP 事件缺口。
- P1：策略三 parser 质量池已导出，优先处理 14 个 `pdf_without_parsed_overseas` 和 13 个 `parse_warning`。
...
python3 -m src.pipeline.cli refresh --valuation-source akshare --industries "休闲、生活及专业服务" --limit 50
# 将上一步实际处理的代码原样传入，避免 --limit 重新取到另一批股票
python3 -m src.pipeline.cli screen --period 2026Q1 --strategy consumer --codes <同一批代码>
python3 -m src.pipeline.cli parser-review --year 2025
python3 -m src.pipeline.cli pharma-screen --period 2025A --codes 300003 002901 688085 603658
...
# 策略一补历史 PE/PB：财务仍走 $a-stock-data，估值走 AkShare/东财历史序列
python3 -m src.pipeline.cli refresh --valuation-source akshare --industries 食品饮料 家电 "休闲、生活及专业服务" --limit 50
```
...
腾讯只提供当前 PE/PB 快照，不伪造 3/5/10 年历史分位；策略一需要历史分位时
请显式使用 `--valuation-source akshare` 分批预热。

### 策略三 parser 质量池

```bash
python3 -m src.pipeline.cli parser-review --year 2025
cat data/exports/overseas_parser_quality_review.md
```
...
| **P1/P2 第二轮收尾**（2026-07-09）| `refresh` 新增 `--valuation-source akshare` 与历史样本跳过阈值，策略一首批 31 只消费样本完成历史 PE/PB + 财务闭环；策略一百分比口径兼容 20.0/0.20，扣非缺失时只用归母/净利做代理 triage；新增 `pharma-review` ground truth 复盘与 `parser-review` 海外收入质量池 |
| **策略一营运质量守卫**（2026-07-10）| 新浪三表摘要/`financials` 新增应收账款、存货、销售费用；策略一以同报告期同比衡量应收/存货相对收入增速和销售费用率变化，输出独立 watch/reject 原因码；修复季度毛利率误与上一年年报对比的问题；财务默认窗口收敛为 9 期，只覆盖 2024Q1 至 2026Q1；历史 run 按目标报告期截断财务序列，避免未来数据穿越 |
| **策略一消费池预热第二轮**（2026-07-11）| 食品饮料与家电各完成 50 只历史 PE/PB + 财务预热，消费池达标样本增至 130 只；`refresh` 将不足策略阈值的历史样本显式输出为 `insufficient`，避免批量补数误报成功 |
| **策略一行业映射与预热第三轮**（2026-07-11）| 策略一改为多字段消费行业准入，修复美护/个护、纺服和服务消费在东财标签下的漏选；美护/个护 20 只、纺服 50 只预热完成，消费池历史 PE/PB 达标样本增至 199/684；`refresh` 按持久化去重后的历史样本数判定 `insufficient` |
...
| **P1 策略一营运质量过滤**（2026-07-10）| `financials` 新增应收账款/存货/销售费用；同比背离采用去年同报告期，避免季节性误判。应收/存货/销售费用率分别输出独立 watch/reject 原因码；字段缺失仅在 metrics 标记，避免未回填历史样本被误杀 |
